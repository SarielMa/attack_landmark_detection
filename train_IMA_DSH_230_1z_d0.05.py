...
def classify_model_std_output_reg(heatmap, guassian_mask, regression_y, offset_y, regression_x, offset_x, mask, threshold="2z"):
    ...
    Yp_e_Y=(de<=threshold1) 
    return Yp_e_Y
#
def classify_model_adv_output_reg(heatmap, guassian_mask, regression_y, offset_y, regression_x, offset_x, mask, threshold="1z"):

    #threshold1 = 0.38177521217459864 for 400x320
    # for 200 x 160 train 2Z
    #threshold1 = 0.15468299836652946
    # val 2z
    #threshold1 = 0.42552396298049644
    # train 1z
    threshold1 = 0.04201499497049031
    threshold2 = 0.032557159754742424
    threshold3 = 0.027147733827929565
    de, ry, rx=l1_matric_dice(heatmap, guassian_mask, regression_y, offset_y, regression_x, offset_x, mask)  
    Yp_e_Y=(de<=threshold1)&(ry<=threshold2)&(rx <= threshold3)
    return Yp_e_Y

def IMA_update_margin(E, delta, max_margin, flag1, flag2):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", default='try_5', help="name of the run")
    ...
    MRE_list = list()
    #config['num_epochs']
    num_iternations = int(args.iterations)
    for epoch in range(num_iternations):
        loss_list = list()
        ...
        for img, mask, guassian_mask, offset_y, offset_x, landmark_list, idx in tqdm(dataloader):
         ...
         #--------------------update the margins
            ...
            #flag2[idx]=1
        #-----------------------------------------------------------------------
        if epoch < num_iternations:
            IMA_update_margin(E, delta, noise, flag1, flag2) 
