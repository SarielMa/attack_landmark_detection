...
def run_model_adv_reg(net, img, mask, offset_y, offset_x, guassian_mask, return_loss=False, reduction='none'):
    ...
    if return_loss == True:
        ...
        return heatmap, regression_y, regression_x
#
def classify_model_std_output_reg(heatmap, guassian_mask, regression_y, offset_y, regression_x, offset_x, mask, threshold="mean"):
    if threshold  == "min":
        threshold1=0.9076504
    else:
        threshold1 = 0.9597466106302244
    ...
    return Yp_e_Y
#
def classify_model_adv_output_reg(heatmap, guassian_mask, regression_y, offset_y, regression_x, offset_x, mask, threshold="mean"):
    if threshold  == "min":
        threshold1=0.9076504
    else:
        threshold1 = 0.9597466106302244
